...
public class AllMiniLmL6V2Tests
{
    ...
    public void MultipleSameTest()
    {
        ...
        Assert.IsTrue(embeddings.Any());
        Assert.AreEqual(sentences.Length, embeddings.Count());

        // Make sure all embeddings are the same.
        Assert.IsTrue(embeddings.All(e => e.SequenceEqual(embeddings.First())));
        Assert.IsTrue(embeddings.All(e => e.SequenceEqual(embedding)));
        foreach (var batchEmbedding in embeddings)
        {
            Assert.IsTrue(batchEmbedding.SequenceEqual(embedding),
    ...
    public void MultipleSamev2Test()
    {
        ...
        for (var i = 0; i < sentences.Length; i++)
        {
            ...
            if(i == 0 || i == 2)
            {
                Assert.IsFalse(batch.SequenceEqual(embedding));
            }
            else
            {
                Assert.IsTrue(batch.SequenceEqual(embedding));
            }
        }
    }
}
